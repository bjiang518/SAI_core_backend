...
class PDFGeneratorService: ObservableObject {
    ...
    /// Generate PDF for Pro Mode homework using ProModePDFExporter
    /// Shows only questions and cropped images (no grading information)
    /// Uses the same rendering engine as DigitalHomeworkViewModel for consistency
    func generateProModePDF(
        ...
        // ✅ Use ProModePDFExporter (same as DigitalHomeworkViewModel)
        let pdfExporter = ProModePDFExporter()
        ...
        // Generate PDF using ProModePDFExporter
        let pdfDocument = await pdfExporter.exportToPDF(
            questions: digitalHomework.questions,
            subject: subject,
            totalQuestions: digitalHomework.parseResults.totalQuestions,
