...
class ReportAvailabilityChecker: ObservableObject {
    ...
    func startPeriodicChecking() {
        debugPrint("📊 ReportAvailabilityChecker: Starting periodic checking (every \(Int(checkInterval / 60)) minutes)")
    ...
    func stopPeriodicChecking() {
        debugPrint("📊 ReportAvailabilityChecker: Stopping periodic checking")
    ...
    func checkForNewReports() async {
        debugPrint("📊 ReportAvailabilityChecker: Checking for new reports...")

        // Check if enough time has passed since last check (prevent spam)
        if !shouldCheckNow() {
            debugPrint("📊 ReportAvailabilityChecker: Skipping check (too soon since last check)")
            return
        }
        ...
        do {
            ...
            if !newBatches.isEmpty {
                debugPrint("📊 ReportAvailabilityChecker: Found \(newBatches.count) new report batch(es)")
                ...
                debugPrint("📊 ReportAvailabilityChecker: No new reports found")
            }
            ...
            debugPrint("📊 ReportAvailabilityChecker: Failed to check for new reports: \(error)")
        }
    }
    ...
    private func sendNotificationForBatch(_ batch: ReportBatch) {
        debugPrint("📊 ReportAvailabilityChecker: Sending notification for \(batch.period) report")
    ...
    private func loadLastKnownBatchIds() {
        if let stored = UserDefaults.standard.stringArray(forKey: lastKnownBatchIdsKey) {
            ...
            debugPrint("📊 ReportAvailabilityChecker: Loaded \(lastKnownBatchIds.count) known batch IDs")
        }
    }

    private func saveLastKnownBatchIds() {
        UserDefaults.standard.set(Array(lastKnownBatchIds), forKey: lastKnownBatchIdsKey)
        debugPrint("📊 ReportAvailabilityChecker: Saved \(lastKnownBatchIds.count) known batch IDs")
    }
}
