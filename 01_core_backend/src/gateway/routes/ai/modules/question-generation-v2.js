...
module.exports = async function (fastify, opts) {
  ...
  fastify.post('/api/ai/generate-questions/practice', {
    ...
    const { subject, topic, difficulty, count = 5, language = 'en', force_assistants_api, force_ai_engine } = request.body;
    ...
    try {
      if (useAssistantsAPI) {
        ...
        try {
          result = await generateQuestionsWithAssistant(userId, subject, topic, difficulty, count, language);
        } catch (error) {
  ...
  fastify.post('/api/ai/generate-questions/mistakes', async (request, reply) => {
    ...
    // For now, forward to unified endpoint
    // In future, we can add mistake analysis to additional_instructions
    return await fastify.inject({
      method: 'POST',
      url: '/api/ai/generate-questions/practice',
      headers: request.headers,
      payload: {
        subject,
        count,
        // The assistant will call get_common_mistakes automatically
      }
    });
  });
 ...
 */
async function generateQuestionsWithAssistant(userId, subject, topic, difficulty, count, language) {
  const startTime = Date.now();
  ...
  try {
    ...
    const requestMessage = buildPracticeRequestMessage(subject, topic, difficulty, count, language, userId);
 ...
 */
function buildPracticeRequestMessage(subject, topic, difficulty, count, language, userId) {
  const languageMap = {
    'en': 'English',
