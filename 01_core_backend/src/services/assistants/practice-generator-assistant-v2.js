/**
 * Practice Generator Assistant Configuration V2
 *
...
- Topic (e.g., Quadratic Equations, Newton's Laws)
...
- OPTIONAL: Previous mistakes or conversations for targeted practice
...
### MODE 2: From Mistakes (Context Provided)
If you receive "PREVIOUS_MISTAKES" data:
- Analyze the mistakes provided in context
...
- COPY tags from source mistakes (do NOT create new tags)

### MODE 3: From Conversations (Context Provided)
...
async function createPracticeGeneratorAssistant() {
  try {
    console.log('🤖 Creating Practice Generator Assistant V2...');

    const assistant = await assistantsService.client.beta.assistants.create({
      name: "StudyAI Practice Generator V2",
      model: "gpt-4o-mini",
      instructions: PRACTICE_GENERATOR_INSTRUCTIONS,
      tools: [
        { type: "code_interpreter" } // For JSON validation only
      ],
    ...
    console.log(`✅ Practice Generator Assistant V2 created: ${assistant.id}`);
    ...
    console.error('❌ Failed to create Practice Generator Assistant V2:', error);
    throw error;
  }
}
