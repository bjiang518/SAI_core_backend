-- Migration: Add OpenAI Assistants API support (FIXED VERSION)
-- Date: 2025-11-12
-- Description: Add fields to support OpenAI Assistants API integration
-- This version is safer and checks for column existence before creating indexes
...
-- ============================================
-- 7. Update existing tables (SAFE - only if columns exist)
-- ============================================

-- Add index for faster student performance lookups (only if table exists)
DO $$
BEGIN
  -- Check if subject_progress table exists
  IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'subject_progress') THEN
    CREATE INDEX IF NOT EXISTS idx_progress_user_subject ON subject_progress(user_id, subject);
  END IF;

  -- Check if questions table exists and has is_correct column
  IF EXISTS (
    SELECT FROM information_schema.columns
    WHERE table_name = 'questions' AND column_name = 'is_correct'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_questions_user_subject_incorrect
    ON questions(user_id, subject, is_correct) WHERE is_correct = FALSE;
  END IF;
END $$;
...
-- ============================================
-- Migration Complete - Print summary
-- ============================================
...
BEGIN
  ...
  FROM pg_tables
  WHERE tablename IN ('assistants_config', 'openai_threads', 'assistant_metrics', 'daily_assistant_costs', 'function_call_cache');

  -- Count created indexes
  SELECT COUNT(*) INTO index_count
  FROM pg_indexes
  WHERE indexname LIKE 'idx_assistant%' OR indexname LIKE 'idx_openai%' OR indexname LIKE 'idx_function%';

  -- Count created functions
  SELECT COUNT(*) INTO function_count
  FROM pg_proc
  WHERE proname IN ('cleanup_expired_threads', 'update_daily_costs');

  RAISE NOTICE 'Migration completed successfully!';
  RAISE NOTICE 'Tables created/verified: %', table_count;
  RAISE NOTICE 'Indexes created: %', index_count;
  RAISE NOTICE 'Functions created: %', function_count;
  RAISE NOTICE 'Ready to create Assistants!';
END $$;
